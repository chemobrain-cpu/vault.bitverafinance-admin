import {
    ...
    UPDATE_ADMIN, FETCH_DEPOSIT_HANDLERS, DELETE_DEPOSIT_HANDLER, UPDATE_DEPOSIT_HANDLER, CREATE_DEPOSIT_HANDLER, CREATE_COPYTRADE, FETCH_COPYTRADE, DELETE_COPYTRADE,UPDATED_COPYTRADE
...
export const userAuthReducer = (state = initialState, action) => {
    switch (action.type) {
        ...
        case CREATE_COPYTRADE:
            return {
                ...state,
                copyTradesList: [...state.copyTradesList, action.payload]
            };

        case FETCH_COPYTRADE:
            return {
                ...state,
                copyTradesList: action.payload
            };

        case DELETE_COPYTRADE: {
            const copyTradeId = action.payload;
            const newCopyTradeList = state.copyTradesList.filter(ct => ct._id !== copyTradeId);
            return {
                ...state,
                copyTradesList: newCopyTradeList
            };
        }

        case UPDATED_COPYTRADE: {
            const updatedCopyTrade = action.payload;
            const newCopyTradeList = state.copyTradesList.map(ct =>
                ct._id === updatedCopyTrade._id ? updatedCopyTrade : ct
            );
            return {
                ...state,
                copyTradesList: newCopyTradeList
